...
func init() {
	m.Register(func(db dbx.Builder) error {
		...
		collection.CreateRule = types.Pointer("(@request.auth.powers:each ?= \"events\" && @request.auth.id = owner) || (@request.auth.powers:each ?= \"super\")")

		collection.UpdateRule = types.Pointer("(@request.auth.powers:each ?= \"events\" && @request.auth.id = owner) || (@request.auth.powers:each ?= \"super\")")

		collection.DeleteRule = types.Pointer("(@request.auth.powers:each ?= \"events\" && @request.auth.id = owner) || (@request.auth.powers:each ?= \"super\")")
